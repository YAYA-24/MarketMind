...
📄 **完整总结**：见 [PROJECT_SUMMARY.md](PROJECT_SUMMARY.md)（项目介绍、功能、技术栈、一步步优化历程）
...
RAG 使用 BGE Embedding + 混合检索（Dense + BM25）+ Cross-Encoder Rerank。首次导入会下载模型。可选环境变量：`EMBEDDING_MODEL=bge`、`ENABLE_HYBRID_SEARCH=1`、`ENABLE_RERANK=1`（Top 20 → Rerank → Top 5）、`ENABLE_QUERY_EXPANSION=1`。
